...
run = (
    Path.home()
    / "turtlebot3_ws/metrics/experiments/rtab_yolo"
...
pgm = figdir / f"rtab_yolo_run{exp:02d}_map.pgm"
yaml_file = figdir / f"rtab_yolo_run{exp:02d}_map.yaml"
...
save(
    fig,
    f"RTAB_YOLO_Run{exp:02d}_Occupancy_Map_PUBLICATION"
)
...
ax.plot(
    ...
    sa[:, 2],
    linewidth=1.9,
    label="RTAB-Map estimate (YOLO active)"
)
...
save(
    fig,
    f"RTAB_YOLO_Run{exp:02d}_Map_and_Trajectory_PUBLICATION"
)
...
save(
    fig,
    f"RTAB_YOLO_Run{exp:02d}_GT_vs_Estimated_PUBLICATION"
)

print("==============================================")
print(f"RTAB+YOLO Experiment {exp:02d}")
